...
export default function ImoveisAluguel() {
  ...
  useEffect(() => {
    fetch(`${API}/api/imoveis/lista/`)
      .then(r => r.json())
      .then(data => setImoveis(data.filter(im => im.ativo !== false && im.finalidade === "Aluguel")))
      .catch(() => {})
    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          ...
          <Link
            href="/"
            className="flex items-center gap-1.5 text-slate-600 hover:text-emerald-600 text-xs font-medium transition-colors"
          >
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8">
          <p className="text-emerald-600 text-xs font-semibold tracking-widest uppercase mb-1">Disponíveis para locação</p>
          <h1 className="text-2xl font-extrabold text-slate-900">Imóveis para Aluguel</h1>
          {!carregando && (
        ...
        {carregando ? (
          ...
          <div className="py-24 text-center text-slate-500 text-sm">
            Nenhum imóvel para aluguel cadastrado ainda.
          </div>
        ) : (
            ...
            {imoveis.map(im => (
              <div key={im.id} className="flex items-start gap-4 py-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-emerald-600 font-extrabold text-lg mb-2">{formatarPreco(im.preco)}</p>
                ...
                <div className="shrink-0 self-center">
                  <Link
                    href={`/imoveis/${im.id}`}
                    className="border border-emerald-500 text-emerald-500 hover:bg-emerald-600/10 rounded-xl px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap"
                  >
                    Ver detalhes
                  </Link>
                </div>
